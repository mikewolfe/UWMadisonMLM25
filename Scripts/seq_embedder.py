...
arg_parser.add_argument('data_file', type=str, help='path to data file of variants')
arg_parser.add_argument('out_dir', type=str, help='output directory')
arg_parser.add_argument('--batch_size', type=int, help='number of variants to embed at once. lower if OOM error',
                      default=20)
...
if use_gpu:
    print("Using GPU for esm2")
    model.to(device='cuda')
else:
    print("No GPU default to CPU")
...
for i, batch in enumerate(batched_vnts):
    with torch.no_grad():
        if use_gpu:
            gpu_batch= {key:val.to(device='cuda') for key, val in batch.items()}
            gpu_orig_batch = {{key:val.to(device='cuda') for key, val in batched_orig[i].items()}}
            model_out_var = model(**gpu_batch)
            model_out_orig = model(**gpu_orig_batch)
        else:
    ...
    for seq_ind, pos in enumerate(batched_pos[i]):
        batch_mut_aa_emb.append(model_out_var.last_hidden_state[seq_ind, pos, :].detach().cpu())
        batch_orig_aa_emb.append(model_out_orig.last_hidden_state[seq_ind, pos, :].detach().cpu())
    ...
    batched_vnt_emb.append(model_out_var.pooler_output.detach().cpu())
    batched_orig_emb.append(model_out_orig.pooler_output.detach().cpu())
    print(f'finished batch {i}')
    del model_out_var
    del model_out_orig

np_vnt_embs = torch.cat(batched_vnt_emb, dim=0).numpy()
np.savetxt(os.path.join(out_dir, "var_embs.csv"), np_vnt_embs,
           delimiter=',')
np_orig_embs = torch.cat(batched_orig_emb, dim=0).numpy()
np.savetxt(os.path.join(out_dir, "orig_embs.csv"), np_orig_embs,
           delimiter=',')
np_vnt_aa_embs = torch.cat(batched_mut_aa_emb, dim=0).numpy()
np.savetxt(os.path.join(out_dir, "var_aa_embs.csv"), np_vnt_aa_embs,
           delimiter=',')
np_orig_aa_embs = torch.cat(batched_orig_aa_emb, dim=0).numpy()
