...
```{r}
# can also read 2025-09-27_XGboost_results_noseqs.tsv.gz here for the same effect
...
```
```{r}
train_d <- read_csv("../../../data/mave-db-amino-acid-substitution-prediction/train.csv")
train_d %>% filter(ref_short == "W", alt_short == "W")
train_d %>% filter(ref_short == "M", alt_short == "M")
```


```{r}
d_perf %>% select(-seq) %>% write_tsv("2025-09-27_XGboost_results_noseqs.tsv.gz")
```
